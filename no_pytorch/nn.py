"""Tiny nn API (Linear/Embedding/LayerNorm/softmax/cross_entropy/AdamW) on the
no-PyTorch autograd Tensor. Same module shapes as the torch.nn model so the example
reads the same. Reuses tensor.py (which in turn reuses ops.py / pjrt.py)."""
